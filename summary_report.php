<?php
	function generateRow(){
		...
		concat(customer.last_name,', ',customer.first_name,' ',substr(customer.middle_name,1,1),'.') as customer_name,
		...
		from product, sales, sale_details, customer 
		where invoice_date between '".date('Y/m/d',strtotime($_REQUEST["date_from"]))."'
		and '".date('Y/m/d',strtotime($_REQUEST["date_to"]))."' and sales.customer_id = customer.customer_id 
		and sale_details.product_id = product.product_id and sales.sale_id = sale_details.sale_id 
		group by sales.customer_id order by invoice_date,invoice_number";
    ...
    $content .= '
		<h2 align="center">Sales Summary Report</h2>
      	<table border="1" cellspacing="0" cellpadding="3">  
           <tr>  
                <th text-align="center" width="10%">Invoice #</th>
				<th text-align="center" width="14%">Invoice Date</th>
				<th text-align="center" width="22%">Customer</th>
